...
def encode_hand_sequence_eepose(
    ...
    with torch.no_grad():
        ...
        # Semantic alignment: midpoint of virtual thumb (tips[0]) and virtual
        # finger centroid (mean of tips[1:]).  Works for any tip count >= 2
        # and matches the semantic loss used during training.
        virtual_thumb = fingertip_positions[:, 0, :]
        virtual_fingers = fingertip_positions[:, 1:, :].mean(dim=1)
        alignment = 0.5 * (virtual_thumb + virtual_fingers)
